import { Swiper, SwiperSlide } from "swiper/react";
import "swiper/css";
import "swiper/css/pagination";
import { Autoplay, Pagination } from "swiper/modules";
import ItemsReview from "./ItemsReview";
import "./slides.css"

const CardSlides = () => {
    let photo = "data:image/jpeg;base64,"
  const dataReview = [
    {
      name: "Juan Pérez",
      city: "Madrid",
      country_code: "ES",
      photo,
      rating: 4.5,
      rating_text:
        "Excellent service, very satisfied with the attention received. I would definitely choose this place again.",
      date: "2024-07-30",
    },
    {
      name: "Ana Gómez",
      city: "Barcelona",
      country_code: "ES",
      photo,
      rating: 3.8,
      rating_text:
        "Good experience overall, although the wait time was a bit long.",
      date: "2024-07-29",
    },
    {
      name: "Carlos Fernández",
      city: "Valencia",
      country_code: "ES",
      photo,
      rating: 5.0,
      rating_text:
        "An unforgettable experience. Everything was perfect, from the service to the final product.",
      date: "2024-07-28",
    },
    {
      name: "Emily Johnson",
      city: "New York",
      country_code: "US",
      photo,
      rating: 4.0,
      rating_text:
        "Great ambiance and friendly staff. The food was good but could be improved.",
      date: "2024-07-27",
    },
    {
      name: "Liam Smith",
      city: "Los Angeles",
      country_code: "US",
      photo,
      rating: 4.7,
      rating_text:
        "Fantastic experience overall. The service was quick and the place was very clean.",
      date: "2024-07-26",
    },
    {
      name: "Maria Rodriguez",
      city: "Mexico City",
      country_code: "MX",
      photo,
      rating: 4.2,
      rating_text:
        "Good quality, but the portion sizes were smaller than expected.",
      date: "2024-07-25",
    },
    {
      name: "David Lee",
      city: "Toronto",
      country_code: "CA",
      photo,
      rating: 3.5,
      rating_text:
        "The service was decent, but the food did not meet my expectations.",
      date: "2024-07-24",
    },
    {
      name: "Sophia Brown",
      city: "London",
      country_code: "GB",
      photo,
      rating: 4.8,
      rating_text:
        "Absolutely wonderful! The staff was incredibly welcoming and the experience was top-notch.",
      date: "2024-07-23",
    },
    {
      name: "James Wilson",
      city: "Paris",
      country_code: "FR",
      photo,
      rating: 3.9,
      rating_text:
        "Nice place, but there were some issues with the order being incorrect.",
      date: "2024-07-22",
    },
    {
      name: "Isabella Martinez",
      city: "Buenos Aires",
      country_code: "AR",
      photo,
      rating: 4.6,
      rating_text:
        "Very good experience overall. The atmosphere was pleasant and the staff was attentive.",
      date: "2024-07-21",
    },
  ];

  return (
    <>
      <Swiper
        slidesPerView={3}
        spaceBetween={30}
        pagination={{
          clickable: true,
        }}
        modules={[Pagination, Autoplay]}
        className="mySwiper mySwiperCards"
        autoplay={{
          delay: 2000,
          disableOnInteraction: false,
        }}
      >
        {dataReview.map((o, i) => (
          <SwiperSlide key={i} className="slide_card_review relative">
            <ItemsReview data={o} />
          </SwiperSlide>
        ))}
      </Swiper>
    </>
  );
};

export default CardSlides;
